...
> **FullGUI shown above.** FullGUI and LiteGUI share the same application state, market-data/services layer, persistence, alerts, and integrated ticker-search implementation.
...
<p align="center">
  <img src="docs/images/squarestar-search-demo.gif" alt="SquareStar LiteGUI ticker search demo" width="800">
  <br><sub><strong>LiteGUI search demo.</strong> The compact LiteGUI surface uses the same integrated ticker-search implementation and provider/service path as FullGUI.</sub>
</p>
...
> **Chart controls:** right-click a stock chart to change the line style, crosshair behavior, and time labels, enter VS/Monitor modes, or export the GUI image/source data.
...
> **Windows security note:** the executable is currently unsigned, so Windows SmartScreen may warn on first launch. Release downloads include a matching SHA-256 checksum, and the source/build instructions are available in this repository.
...
> **Data-provider note:** SquareStar is an independent client, not an official Yahoo or Finnhub product. Yahoo-backed data is keyless; Finnhub is optional and no Finnhub key ships with the program.

Sensitive persisted state is protected with Windows DPAPI.
