...
## 1.0.1 — Unreleased
- Simplified Full GUI stock navigation to regular in-app tabs, removed independent stock-window behavior, and kept Monitor mode as the dedicated multi-stock view.
- Updated the README screenshots for the current price-alert and monitor workflows.
- Protected ImGui window/layout state with DPAPI alongside other private state, so ticker-bearing window IDs are no longer stored in plaintext `config.json`; the unreleased plaintext layout format has no migration path.
...
- Fixed `Ctrl+K` on a terminal containing one stock so it focuses that stock's terminal search bar instead of falling back to Home.
...
- Made shared foreground notification cards surface-aware so an active toast cannot overflow into the LiteGUI title bar when switching into the 108 px compact search surface.
