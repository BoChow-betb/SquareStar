...
// SquareStar uses a desktop OpenGL 3.3 context and stores its dynamic font
// atlas as an 8-bit coverage mask. Some legacy Windows GL headers used by the
// backend loader don't expose the 3.x texture/swizzle constants at compile time.
...
void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        const GLenum pixel_format = alpha8 ? GL_RED : GL_RGBA;
        const GLint internal_format = alpha8 ? GL_R8 : GL_RGBA;
#else
        const bool alpha8 = false;
        const GLenum pixel_format = GL_RGBA;
        const GLint internal_format = GL_RGBA;
#endif
        IM_ASSERT(tex->Format == ImTextureFormat_RGBA32 || alpha8);
        ...
        if (alpha8)
        {
            // Make an R8 coverage texture sample exactly like the historical
            // RGBA font atlas: RGB=white and A=coverage.
...
#endif
        GL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, internal_format, tex->Width, tex->Height, 0, pixel_format, GL_UNSIGNED_BYTE, pixels));
    ...
    else if (tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        for (ImTextureRect& r : tex->Updates)
            GL_CALL(glTexSubImage2D(GL_TEXTURE_2D, 0, r.x, r.y, r.w, r.h, pixel_format, GL_UNSIGNED_BYTE, tex->GetPixelsAt(r.x, r.y)));
#else
        ...
        for (ImTextureRect& r : tex->Updates)
        {
            ...
            GL_CALL(glTexSubImage2D(GL_TEXTURE_2D, 0, r.x, r.y, r.w, r.h, pixel_format, GL_UNSIGNED_BYTE, bd->TempBuffer.Data));
        }
#endif
        tex->SetStatus(ImTextureStatus_OK);
