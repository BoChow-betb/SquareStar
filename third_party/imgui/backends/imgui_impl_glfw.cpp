...
void ImGui_ImplGlfw_UpdateMouseCursor()
{
    ...
    for (int n = 0; n < platform_io.Viewports.Size; n++)
    {
        ...
        {
            // Hide every ImGui platform window together. LastMouseCursor is shared
            // backend state, so updating it inside this loop would only update the
            // first viewport and leave the others with a stale native cursor.
    ...
    if (imgui_cursor == ImGuiMouseCursor_None || io.MouseDrawCursor)
        ...
        bd->LastMouseCursor = bd->MouseCursors[imgui_cursor] ? bd->MouseCursors[imgui_cursor] : bd->MouseCursors[ImGuiMouseCursor_Arrow];
#endif
}
